...
object MediaEntity extends Entity[Long,SurrogateLongId, Media]{
  ...
  new Media(publishingDate, author, title, subtitle, identifier, description, consumed, showId) with Stored  {
...
val dataSource = play.api.db.DB.getDataSource("default")
  ...
  def upcoming(limit : Int = 5) : Seq[Media] =  {
...
object TVShowEntity extends Entity[Unit, NoId, TVShow]{
  ...
  def constructor(implicit m: ValuesMap) = new TVShow(title, count) with Stored
}
