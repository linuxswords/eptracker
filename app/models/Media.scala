...
object Media {
  ...
  val(jdbc, mapperDao, queryDao, transcationManager) = Setup.h2(dataSource, List(MediaEntity, TVShowEntity))
  val me = MediaEntity
  val ts = TVShowEntity
...
//        .on('title -> title)
  ...
  def recent(limit : Int = 4) : Seq[Media] =  {
    ...
    queryDao.query(QueryConfig(limit = Some(limit)), select from me where me.publishingDate <= today orderBy (me.publishingDate, asc))
  }
...
object TVShowEntity extends Entity[LongId, TVShow]{
  val id = key("id") autogenerated (_.id)
  val title = column("title") to (_.title)
  val count = column("count") to (_.count)

  def constructor(implicit m: ValuesMap) =
    new TVShow(title, count) with LongId with Persisted  {
      val id: Long = TVShowEntity.id
    }
}
